defmodule StreamflixWebWeb.PlatformDashboard.TabComponents do
  ...
  def render_settings_tab(assigns) do
    ...
    <%!-- Row 3: Notification Channels (full width) --%>
    <section class="bg-white dark:bg-slate-800 rounded-xl border border-amber-200 dark:border-amber-800 shadow-sm p-4 sm:p-6 lg:p-8 overflow-hidden">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-2">
          <.icon name="hero-bell-alert" class="w-5 h-5 text-amber-600 dark:text-amber-400" />
          <h2 class="text-base lg:text-lg font-semibold text-slate-900 dark:text-slate-100">
            {gettext("Alertas externas")}
          </h2>
        </div>
        <div class="flex items-center gap-2">
          <%= if @notification_channel do %>
            <button
              type="button"
              phx-click="test_notification_channel"
              phx-disable-with={gettext("Enviando...")}
              class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-lg hover:bg-amber-100 dark:hover:bg-amber-900/40 transition"
            >
              <.icon name="hero-paper-airplane" class="w-3.5 h-3.5" />
              {gettext("Enviar prueba")}
            </button>
            <button
              type="button"
              phx-click="delete_notification_channel"
              phx-disable-with={gettext("Eliminando...")}
              class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-red-600 dark:text-red-400 hover:text-red-700 transition"
              data-confirm={gettext("¿Eliminar la configuración de alertas?")}
            >
      ...
      <p class="text-sm text-slate-500 dark:text-slate-400 mb-4">
        {gettext(
          "Recibe notificaciones cuando algo falla: webhook caídos, entregas en DLQ, jobs fallidos y más."
        )}
      </p>
      <%= if can_manage_team?(@current_user_role) do %>
        <.form
          for={%{}}
          id="nc-form"
          phx-submit="save_notification_channel"
          phx-change="nc_form_change"
          class="space-y-5"
        >
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div class="flex items-center justify-between mb-3">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    name="email_enabled"
                    value="true"
                    checked={@nc_form["email_enabled"]}
                    class="sr-only peer"
                  />
                  <div class="w-9 h-5 bg-slate-300 peer-checked:bg-amber-500 rounded-full after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-full">
                  </div>
                </label>
              </div>
              <input
            ...
            <div class="rounded-lg border border-slate-200 dark:border-slate-700 p-4">
              <div class="flex items-center justify-between mb-3">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm font-medium text-slate-800 dark:text-slate-200">Slack</span>
                </div>
                <label class="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
              ...
              <input
                ...
                value={@nc_form["slack_webhook_url"]}
            ...
            <div class="rounded-lg border border-slate-200 dark:border-slate-700 p-4">
              <div class="flex items-center justify-between mb-3">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    value="true"
                    checked={@nc_form["discord_enabled"]}
                    class="sr-only peer"
                  />
                  <div class="w-9 h-5 bg-slate-300 peer-checked:bg-amber-500 rounded-full after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-full">
                  </div>
              ...
              <input
                ...
                value={@nc_form["discord_webhook_url"]}
            ...
            <div class="rounded-lg border border-slate-200 dark:border-slate-700 p-4">
              <div class="flex items-center justify-between mb-3">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    name="meta_webhook_enabled"
                    value="true"
                    checked={@nc_form["meta_webhook_enabled"]}
                    class="sr-only peer"
                  />
                  <div class="w-9 h-5 bg-slate-300 peer-checked:bg-amber-500 rounded-full after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-full">
                  </div>
              ...
              <input
                ...
                value={@nc_form["meta_webhook_url"]}
                placeholder="https://myapp.com/jobcelis-alerts"
                class="w-full border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 rounded-lg px-3 py-2 text-sm mb-2"
              />
              <input
                type="text"
                name="meta_webhook_secret"
                value={@nc_form["meta_webhook_secret"]}
                placeholder={gettext("Secret (opcional, para firma HMAC)")}
                class="w-full border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 rounded-lg px-3 py-2 text-sm"
              />
            </div>
          </div>
          ...
          <div class="rounded-lg border border-slate-200 dark:border-slate-700 p-4">
            <p class="text-sm font-medium text-slate-800 dark:text-slate-200 mb-2">
              {gettext("Tipos de evento (dejar vacío = todos)")}
            </p>
            <div class="flex flex-wrap gap-2">
              <%= for type <- ~w(webhook_failing circuit_open circuit_closed dlq_entry job_failed replay_completed) do %>
                <label class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs cursor-pointer transition hover:bg-slate-50 dark:hover:bg-slate-700/50 border-slate-200 dark:border-slate-700">
                  <input
                    type="checkbox"
                    name="event_types[]"
                    value={type}
                    checked={type in (@nc_form["event_types"] || [])}
          ...
          <%= if @nc_form_errors do %>
            <div class="text-sm text-red-600 dark:text-red-400">
              <%= for {field, msgs} <- @nc_form_errors do %>
                <p><strong>{field}:</strong> {Enum.join(msgs, ", ")}</p>
              <% end %>
            </div>
          <% end %>
          <div class="flex justify-end">
            ...
            >
              ...
              {gettext("Guardar alertas")}
            </button>
          </div>
        </.form>
      <% else %>
        <p class="text-sm text-slate-400 dark:text-slate-500 text-center py-8">
          {gettext("Solo el owner o editor puede configurar las alertas externas.")}
        </p>
      <% end %>
    </section>
