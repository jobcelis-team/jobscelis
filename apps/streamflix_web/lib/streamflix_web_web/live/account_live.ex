defmodule StreamflixWebWeb.AccountLive do
  ...
  def render(assigns) do
    ~H"""
    <Layouts.app flash={@flash} current_scope={:account} current_user={@current_user} locale={@locale}>
      <div class="max-w-2xl">
        <h1 class="text-2xl font-bold text-slate-900 mb-6"><%= gettext("Cuenta") %></h1>
        <section class="bg-white rounded-xl border border-slate-200 shadow-sm p-6" aria-labelledby="account-heading">
          <h2 id="account-heading" class="sr-only"><%= gettext("Datos de tu cuenta") %></h2>
          <dl class="space-y-3">
            <div>
              <dt class="text-sm font-medium text-slate-500"><%= gettext("Email") %></dt>
              <dd class="text-slate-900">{@user.email}</dd>
            </div>
            <%= if @user.role in ["admin", "superadmin"] do %>
              <div>
                <dt class="text-sm font-medium text-slate-500"><%= gettext("Rol") %></dt>
