defmodule StreamflixWebWeb.PlatformDashboardLive do
  ...
  def handle_event("send_test", %{"topic" => topic, "payload" => payload_str} = _params, socket) do
    ...
    socket = case result do
      {:ok, event} ->
        Audit.record("event.created", user_id: socket.assigns.current_user.id, project_id: project.id, resource_type: "event", resource_id: event.id)
  ...
  def handle_event("regenerate_token", _params, socket) do
    ...
    case Platform.regenerate_api_key(project.id) do
      {:ok, _api_key, raw_key} ->
        ...
        socket =
          Audit.record("api_key.regenerated", user_id: socket.assigns.current_user.id, project_id: project.id, resource_type: "api_key")
          socket
  ...
  def handle_event("switch_project", %{"id" => id}, socket) do
    ...
    case Platform.get_project(id) do
      ...
      project ->
        if old_project, do: Phoenix.PubSub.unsubscribe(StreamflixCore.PubSub, "project:#{old_project.id}")
    ...
    case Platform.get_project(id) do
      ...
      project ->
        if project.user_id != user.id do
          {:noreply, put_flash(socket, :error, gettext("Solo el dueño puede eliminar el proyecto."))}
        else
  ...
  def handle_event("delete_event_schema", %{"id" => id}, socket) do
    ...
    case Platform.get_event_schema(id) do
      nil -> {:noreply, put_flash(socket, :error, gettext("Schema no encontrado."))}
      schema ->
  ...
  def handle_event("update_member_role", %{"id" => id, "role" => role}, socket) do
    case Teams.update_member_role(id, role) do
      {:ok, _} ->
        members = Teams.list_members(socket.assigns.project.id)
        {:noreply, socket |> assign(:team_members, members) |> put_flash(:info, gettext("Rol actualizado."))}
      {:error, _} ->
        ...
        <div class="flex items-center justify-between mb-6 sm:mb-8">
          <div class="flex items-center gap-3">
            ...
            <%= if length(@projects) > 1 do %>
              <div class="relative">
                <button type="button" phx-click="toggle_project_selector" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-lg text-sm font-medium transition border border-indigo-200">
                  <.icon name="hero-rectangle-stack" class="w-4 h-4" />
                  <span class="hidden sm:inline truncate max-w-[10rem]"><%= @project && @project.name %></span>
                ...
                <%= if @show_project_selector do %>
                  <div class="fixed inset-x-3 top-20 sm:absolute sm:inset-x-auto sm:top-auto sm:right-0 mt-1 sm:w-72 bg-white rounded-xl shadow-xl border border-slate-200 z-50 max-h-80 overflow-y-auto">
                    <div class="p-3 border-b border-slate-200">
                      <.form for={%{}} id="create-project-form" phx-submit="create_project" class="flex gap-2">
                        <input type="text" name="name" placeholder={gettext("Nuevo proyecto...")} class="flex-1 min-w-0 border border-slate-300 rounded-lg px-3 py-1.5 text-sm" />
                        <button type="submit" class="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-medium"><%= gettext("Crear") %></button>
                      </.form>
                    </div>
                    <%= for p <- @projects do %>
                      <div class={"flex items-center justify-between px-4 py-2.5 hover:bg-slate-50 cursor-pointer transition #{if @project && @project.id == p.id, do: "bg-indigo-50/50"}"} phx-click="switch_project" phx-value-id={p.id}>
                        <div class="min-w-0">
                          <p class="text-sm font-medium text-slate-800 truncate"><%= p.name %></p>
                          <p class="text-[10px] text-slate-400 font-mono"><%= String.slice(p.id, 0, 8) %>...</p>
                        </div>
                        <div class="flex items-center gap-1.5 shrink-0">
                          <%= if p.is_default do %>
                            <span class="px-1.5 py-0.5 rounded text-[10px] font-medium bg-indigo-100 text-indigo-700"><%= gettext("Default") %></span>
                          <% end %>
                          <%= if p.user_id != @current_user.id do %>
                            <span class="px-1.5 py-0.5 rounded text-[10px] font-medium bg-slate-100 text-slate-600"><%= gettext("Miembro") %></span>
                          <% end %>
                        </div>
                      </div>
                    <% end %>
          ...
          <%!-- Notification Bell --%>
          <div class="relative">
            <button type="button" phx-click="toggle_notifications" class="relative p-2 text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition">
              <.icon name="hero-bell" class="w-6 h-6" />
              <%= if @unread_count > 0 do %>
                <span class="absolute -top-0.5 -right-0.5 flex items-center justify-center w-5 h-5 text-[10px] font-bold text-white bg-red-500 rounded-full"><%= @unread_count %></span>
              <% end %>
            </button>
            ...
            <%= if @show_notifications do %>
              <div class="fixed inset-x-3 top-16 sm:absolute sm:inset-x-auto sm:top-auto sm:right-0 mt-2 sm:w-96 bg-white rounded-xl shadow-xl border border-slate-200 z-50 max-h-96 overflow-y-auto">
                <div class="flex items-center justify-between px-4 py-3 border-b border-slate-200">
                  ...
                  <%= if @unread_count > 0 do %>
                    <button phx-click="mark_all_read" class="text-xs text-indigo-600 hover:text-indigo-700 font-medium"><%= gettext("Marcar todo leído") %></button>
                  <% end %>
                </div>
                <%= if @notifications == [] do %>
                  <div class="px-4 py-8 text-center text-sm text-slate-400"><%= gettext("Sin notificaciones") %></div>
                <% else %>
                  <%= for notif <- @notifications do %>
                    <div class={"px-4 py-3 border-b border-slate-100 last:border-0 hover:bg-slate-50 transition cursor-pointer #{if !notif.read, do: "bg-indigo-50/50"}"} phx-click="mark_notification_read" phx-value-id={notif.id}>
                      <div class="flex items-start gap-2">
                        <span class={"mt-1 w-2 h-2 rounded-full shrink-0 #{if !notif.read, do: "bg-indigo-500", else: "bg-transparent"}"}></span>
                        <div class="min-w-0">
                          <p class="text-sm font-medium text-slate-900 truncate"><%= notification_title(notif) %></p>
                          <p class="text-xs text-slate-500 truncate"><%= notification_message(notif) %></p>
                          <p class="text-[10px] text-slate-400 mt-1"><%= format_dt(notif.inserted_at) %></p>
                        </div>
                      </div>
                    </div>
                  <% end %>
          ...
          <div class="space-y-6 sm:space-y-8">
                ...
                <div class="mt-4 pt-4 border-t border-slate-200 space-y-2">
                  <div class="flex items-center gap-2">
                    ...
                    <%= for scope <- (@api_key.scopes || ["*"]) do %>
                      <span class="px-2 py-0.5 bg-indigo-50 text-indigo-700 rounded text-xs font-mono"><%= scope %></span>
                    <% end %>
                  </div>
                  <div class="flex items-start gap-2">
                    <span class="text-xs font-medium text-slate-500 uppercase shrink-0"><%= gettext("IPs permitidas") %>:</span>
                    <%= if @api_key.allowed_ips && @api_key.allowed_ips != [] do %>
                      <div class="flex flex-wrap gap-1">
                        <%= for ip <- @api_key.allowed_ips do %>
                          <span class="px-2 py-0.5 bg-emerald-50 text-emerald-700 rounded text-xs font-mono"><%= ip %></span>
                        <% end %>
                      </div>
                    <% else %>
            ...
            <section class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 sm:p-6 overflow-hidden">
              ...
              <.form for={%{}} id="test-event-form" phx-submit="send_test" class="space-y-3 sm:max-w-lg">
                ...
                <div class="flex gap-2">
                  ...
                  <button type="button" phx-click="simulate_event" phx-value-topic={@test_topic} phx-value-payload={@test_payload} phx-disable-with={gettext("Simulando...")} class="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg font-medium text-sm transition disabled:opacity-70 disabled:cursor-not-allowed">
                    <%= gettext("Simular") %>
                  </button>
                </div>
              </.form>
              ...
              <%= if @simulation_result do %>
                <div class="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                  <div class="flex items-center justify-between mb-3">
                    ...
                    <button type="button" phx-click="close_simulation" class="text-amber-600 hover:text-amber-800 text-sm"><%= gettext("Cerrar") %></button>
                  </div>
                  <%= if @simulation_result == [] do %>
                    <p class="text-sm text-amber-800"><%= gettext("Ningún webhook matchearía con este evento.") %></p>
                  <% else %>
                    <p class="text-sm text-amber-800 mb-2"><%= gettext("%{count} webhook(s) recibirían este evento:", count: length(@simulation_result)) %></p>
                    <%= for sim <- @simulation_result do %>
                      <div class="mt-2 p-3 bg-white border border-amber-100 rounded text-sm">
                        ...
                        <p class="text-xs text-slate-500 mt-1">
                          <%= gettext("Topics") %>: <span class="font-medium"><%= if sim.matched_by_topics, do: "✓", else: "✗" %></span>
                          · <%= gettext("Filtros") %>: <span class="font-medium"><%= if sim.matched_by_filters, do: "✓", else: "✗" %></span>
                          <%= if sim.would_send_headers["x-signature"] do %>
                            · HMAC: <span class="font-mono text-xs"><%= String.slice(sim.would_send_headers["x-signature"], 0, 20) %>...</span>
                          <% end %>
                        </p>
                      </div>
                    <% end %>
            ...
            <section class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 sm:p-6 overflow-hidden">
              <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
                ...
                <div class="flex flex-wrap gap-2">
                  <a href={"/api/v1/export/events?format=csv"} target="_blank" class="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition">
                    <.icon name="hero-arrow-down-tray" class="w-3.5 h-3.5" /> CSV
                  </a>
                  <a href={"/api/v1/export/events?format=json"} target="_blank" class="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition">
                    ...
                    <%= for e <- @events do %>
                      <tr class="border-b border-slate-100 last:border-0">
                        ...
                        <td class="px-3 sm:px-4 py-2 sm:py-3">
                          <span class="px-2 py-0.5 rounded text-xs font-medium bg-slate-100 text-slate-700">{e.status}</span>
                          <%= if e.deliver_at && DateTime.compare(e.deliver_at, DateTime.utc_now()) == :gt do %>
                            <span class="ml-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-amber-100 text-amber-700"><%= gettext("Programado") %></span>
                          <% end %>
                        </td>
                ...
                <table class="min-w-full">
                  <thead>
                    <tr class="bg-slate-50 border-b border-slate-200">
                      ...
                      <th class="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-medium text-slate-700"><%= gettext("Salud") %></th>
                      <th class="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-medium text-slate-700"><%= gettext("Estado") %></th>
                    </tr>
                  </thead>
                  <tbody>
                    <%= for w <- @webhooks do %>
                      ...
                      <tr class="border-b border-slate-100 last:border-0">
                        ...
                        <td class="px-3 sm:px-4 py-2 sm:py-3">
                          <%= if health do %>
                            <span title={"#{health.success_rate}% — #{health.total} deliveries (24h)"} class={"inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium #{health_class(health.score)}"}>
            ...
            <section class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 sm:p-6 overflow-hidden">
              <div class="flex flex-wrap items-center justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
                ...
                <div class="flex flex-wrap gap-2">
                  <a href={"/api/v1/export/jobs?format=csv"} target="_blank" class="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition">
                    <.icon name="hero-arrow-down-tray" class="w-3.5 h-3.5" /> CSV
                  </a>
                  <button
                    type="button"
            ...
            <%= if @dead_letters != [] do %>
              <section class="bg-white rounded-xl border border-red-200 shadow-sm p-4 sm:p-6 overflow-hidden">
                <div class="flex items-center gap-2 mb-4">
                  ...
                  <span class="px-2 py-0.5 rounded-full bg-red-100 text-red-700 text-xs font-medium"><%= length(@dead_letters) %></span>
                </div>
                <p class="text-sm text-red-700 mb-4"><%= gettext("Entregas que agotaron todos los reintentos. Puedes reintentar o descartar.") %></p>
                <div class="overflow-x-auto rounded-lg border border-red-100">
                  <table class="min-w-full divide-y divide-red-100">
                    <thead>
                      <tr class="bg-red-50/50">
                        <th class="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs font-semibold text-red-700 uppercase"><%= gettext("Webhook") %></th>
                        <th class="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs font-semibold text-red-700 uppercase hidden sm:table-cell"><%= gettext("Error") %></th>
                        <th class="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs font-semibold text-red-700 uppercase"><%= gettext("Intentos") %></th>
                        <th class="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs font-semibold text-red-700 uppercase hidden sm:table-cell"><%= gettext("Fecha") %></th>
                        <th class="px-3 sm:px-4 py-2 sm:py-3 text-right text-xs font-semibold text-red-700 uppercase"><%= gettext("Acciones") %></th>
                      </tr>
                    </thead>
                    <tbody class="divide-y divide-red-50">
                      <%= for dl <- @dead_letters do %>
                        <tr class="hover:bg-red-50/30 transition">
                          <td class="px-3 sm:px-4 py-2 sm:py-3 font-mono text-xs text-slate-600 truncate max-w-[10rem] sm:max-w-none"><%= if dl.webhook, do: dl.webhook.url, else: "—" %></td>
                          <td class="px-3 sm:px-4 py-2 sm:py-3 text-xs text-red-700 truncate max-w-[12rem] hidden sm:table-cell"><%= dl.last_error || "—" %></td>
                          <td class="px-3 sm:px-4 py-2 sm:py-3 text-sm text-slate-600"><%= dl.attempts_exhausted %></td>
                          <td class="px-3 sm:px-4 py-2 sm:py-3 text-sm text-slate-600 hidden sm:table-cell"><%= format_dt(dl.inserted_at) %></td>
                          <td class="px-3 sm:px-4 py-2 sm:py-3 text-right">
                            <div class="flex flex-col sm:flex-row sm:justify-end gap-1">
                              <button phx-click="retry_dead_letter" phx-value-id={dl.id} phx-disable-with={gettext("...")} class="text-indigo-600 hover:text-indigo-700 font-medium text-xs disabled:opacity-70"><%= gettext("Reintentar") %></button>
                              <button phx-click="resolve_dead_letter" phx-value-id={dl.id} phx-disable-with={gettext("...")} class="text-slate-500 hover:text-slate-700 font-medium text-xs disabled:opacity-70"><%= gettext("Descartar") %></button>
                            </div>
                          </td>
                        </tr>
                      <% end %>
            ...
            <section class="bg-white rounded-xl border border-blue-200 shadow-sm p-4 sm:p-6 overflow-hidden">
              <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
                ...
                <button type="button" phx-click="open_replay_modal" class="inline-flex items-center gap-2 px-3 sm:px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-medium text-sm shadow-sm transition">
              ...
              <%= if @replays == [] do %>
                <p class="text-sm text-slate-500"><%= gettext("No hay replays. Usa esta función para re-enviar eventos históricos a tus webhooks.") %></p>
              <% else %>
                <div class="overflow-x-auto rounded-lg border border-slate-200">
                  <table class="min-w-full divide-y divide-slate-200">
                    <thead>
                      <tr class="bg-slate-50/80">
                        <th class="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs font-semibold text-slate-600 uppercase"><%= gettext("Estado") %></th>
                        <th class="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs font-semibold text-slate-600 uppercase"><%= gettext("Progreso") %></th>
                        <th class="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs font-semibold text-slate-600 uppercase hidden sm:table-cell"><%= gettext("Filtros") %></th>
                        <th class="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs font-semibold text-slate-600 uppercase hidden sm:table-cell"><%= gettext("Fecha") %></th>
                        <th class="px-3 sm:px-4 py-2 sm:py-3 text-right text-xs font-semibold text-slate-600 uppercase"><%= gettext("Acciones") %></th>
                      </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                      <%= for r <- @replays do %>
                        <tr class="hover:bg-slate-50/50 transition">
                          <td class="px-3 sm:px-4 py-2 sm:py-3">
                            <span class={"inline-flex px-2 py-0.5 rounded text-xs font-medium #{replay_status_class(r.status)}"}><%= replay_status_label(r.status) %></span>
                          </td>
                          <td class="px-3 sm:px-4 py-2 sm:py-3">
                            <div class="flex items-center gap-2">
                              <div class="flex-1 bg-slate-200 rounded-full h-2 max-w-[8rem]">
                                <div class="bg-blue-600 h-2 rounded-full transition-all" style={"width: #{if r.total_events > 0, do: Float.round(r.processed_events / r.total_events * 100, 0), else: 0}%"}></div>
                              </div>
                              <span class="text-xs text-slate-600"><%= r.processed_events %>/<%= r.total_events %></span>
                            </div>
                          </td>
                          <td class="px-3 sm:px-4 py-2 sm:py-3 text-xs text-slate-600 hidden sm:table-cell">
                            <%= if r.filters["topic"], do: "topic: #{r.filters["topic"]}", else: gettext("Todos") %>
                          </td>
                          <td class="px-3 sm:px-4 py-2 sm:py-3 text-sm text-slate-600 hidden sm:table-cell"><%= format_dt(r.inserted_at) %></td>
                          <td class="px-3 sm:px-4 py-2 sm:py-3 text-right">
                            <%= if r.status in ["pending", "running"] do %>
                              <button phx-click="cancel_replay" phx-value-id={r.id} class="text-red-600 hover:text-red-700 font-medium text-xs"><%= gettext("Cancelar") %></button>
                            <% end %>
                          </td>
                        </tr>
                      <% end %>
            ...
            <%= if @replay_modal do %>
              <div class="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6">
                <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" phx-click="close_replay_modal" aria-hidden="true"></div>
                <div class="relative z-10 w-full max-w-lg bg-white rounded-2xl shadow-2xl p-4 sm:p-6 border border-slate-200/50">
                  <div class="flex items-center justify-between mb-4">
                    ...
                    <button type="button" phx-click="close_replay_modal" class="text-slate-400 hover:text-slate-600"><.icon name="hero-x-mark" class="w-5 h-5" /></button>
                  </div>
                  <.form for={%{}} id="replay-form" phx-submit="start_replay" class="space-y-4">
                    <div>
                      <label class="block text-sm font-medium text-slate-700 mb-1"><%= gettext("Topic (opcional)") %></label>
                      <input type="text" name="topic" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" placeholder={gettext("Filtrar por topic")} />
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1"><%= gettext("Desde") %></label>
                        <input type="datetime-local" name="from_date" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
                      </div>
                      <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1"><%= gettext("Hasta") %></label>
                        <input type="datetime-local" name="to_date" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
                      </div>
                    </div>
                    <div class="flex justify-end gap-3 pt-2">
                      <button type="button" phx-click="close_replay_modal" class="px-4 py-2 border border-slate-300 rounded-lg text-slate-700 hover:bg-slate-50 text-sm font-medium"><%= gettext("Cancelar") %></button>
                      <button type="submit" phx-disable-with={gettext("Iniciando...")} class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium"><%= gettext("Iniciar replay") %></button>
                    </div>
                  </.form>
                </div>
              </div>
            ...
            <section class="bg-white rounded-xl border border-purple-200 shadow-sm p-4 sm:p-6 overflow-hidden">
              <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
                ...
                <button type="button" phx-click="create_sandbox" class="inline-flex items-center gap-2 px-3 sm:px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-xl font-medium text-sm shadow-sm transition">
                  <.icon name="hero-plus" class="w-4 h-4" />
                  <%= gettext("Crear endpoint") %>
                </button>
              </div>

              <%= if @sandbox_endpoints == [] do %>
                <p class="text-sm text-slate-500"><%= gettext("Crea un endpoint temporal para recibir y ver requests en tiempo real.") %></p>
              <% else %>
                <div class="flex flex-wrap gap-2 mb-4">
                  <%= for ep <- @sandbox_endpoints do %>
                    <div class={"inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border text-sm cursor-pointer transition #{if @sandbox_active && @sandbox_active.id == ep.id, do: "bg-purple-50 border-purple-300 text-purple-800", else: "bg-white border-slate-200 text-slate-600 hover:bg-slate-50"}"} phx-click="select_sandbox" phx-value-id={ep.id}>
                      <span class="font-mono text-xs"><%= ep.slug %></span>
                      <button type="button" phx-click="delete_sandbox" phx-value-id={ep.id} class="ml-1 text-slate-400 hover:text-red-500" title={gettext("Eliminar")}>
              ...
              <%= if @sandbox_active do %>
                <div class="rounded-lg border border-purple-100 bg-purple-50/50 p-3 mb-4">
                  ...
                  <code class="text-xs font-mono text-purple-900 break-all"><%= sandbox_url(@sandbox_active) %></code>
                </div>

                <%= if @sandbox_requests == [] do %>
                  <p class="text-sm text-slate-500 text-center py-4"><%= gettext("Esperando requests... Envía un POST/GET a la URL de arriba.") %></p>
                <% else %>
                  <div class="overflow-x-auto rounded-lg border border-slate-200">
                    <table class="min-w-full divide-y divide-slate-200">
                      <thead>
                        <tr class="bg-slate-50/80">
                          <th class="px-3 sm:px-4 py-2 text-left text-xs font-semibold text-slate-600 uppercase"><%= gettext("Método") %></th>
                          <th class="px-3 sm:px-4 py-2 text-left text-xs font-semibold text-slate-600 uppercase hidden sm:table-cell"><%= gettext("Body") %></th>
                          <th class="px-3 sm:px-4 py-2 text-left text-xs font-semibold text-slate-600 uppercase hidden sm:table-cell">IP</th>
                          <th class="px-3 sm:px-4 py-2 text-left text-xs font-semibold text-slate-600 uppercase"><%= gettext("Fecha") %></th>
                        </tr>
                      </thead>
                      <tbody class="divide-y divide-slate-100">
                        <%= for req <- @sandbox_requests do %>
                          <tr class="hover:bg-slate-50/50 transition">
                            <td class="px-3 sm:px-4 py-2">
                              <span class={"inline-flex px-2 py-0.5 rounded text-xs font-medium #{method_color(req.method)}"}><%= req.method %></span>
                            </td>
                            <td class="px-3 sm:px-4 py-2 text-xs text-slate-600 font-mono truncate max-w-[16rem] hidden sm:table-cell"><%= if req.body && req.body != "", do: String.slice(req.body, 0, 80), else: "—" %></td>
                            <td class="px-3 sm:px-4 py-2 text-xs text-slate-600 hidden sm:table-cell"><%= req.ip || "—" %></td>
                            <td class="px-3 sm:px-4 py-2 text-xs text-slate-600"><%= format_dt(req.inserted_at) %></td>
                          </tr>
                        <% end %>
                      </tbody>
                    </table>
            ...
            <section class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 sm:p-6 overflow-hidden">
              <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
                ...
                <button type="button" phx-click="toggle_analytics" class="text-sm text-indigo-600 hover:text-indigo-700 font-medium">
              ...
              <%= if @show_analytics and @analytics != %{} do %>
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                  ...
                  <div class="border border-slate-200 rounded-lg p-3 sm:p-4">
                    <h3 class="text-sm font-semibold text-slate-700 mb-3"><%= gettext("Eventos por día (30d)") %></h3>
                    <div class="h-48 sm:h-56">
                      <canvas
                        ...
                        data-chart-datasets={Jason.encode!([%{label: gettext("Eventos"), data: Enum.map(@analytics.events_per_day, & &1.count), borderColor: "#6366f1", backgroundColor: "rgba(99,102,241,0.1)", fill: true, tension: 0.3}])}
                      ></canvas>
                    </div>
                  </div>
                  ...
                  <div class="border border-slate-200 rounded-lg p-3 sm:p-4">
                    <h3 class="text-sm font-semibold text-slate-700 mb-3"><%= gettext("Entregas por día (30d)") %></h3>
                    <div class="h-48 sm:h-56">
                      <canvas
                        id="deliveries-chart"
                        phx-hook="Chart"
                        data-chart-type="bar"
                        data-chart-labels={Jason.encode!(Enum.map(@analytics.deliveries_per_day, & &1.date))}
                        data-chart-datasets={Jason.encode!([
                          %{label: gettext("Exitosas"), data: Enum.map(@analytics.deliveries_per_day, & &1.success), backgroundColor: "#10b981"},
                          %{label: gettext("Fallidas"), data: Enum.map(@analytics.deliveries_per_day, & &1.failed), backgroundColor: "#ef4444"}
                        ])}
                      ></canvas>
                    </div>
                  </div>
                  ...
                  <div class="border border-slate-200 rounded-lg p-3 sm:p-4">
                    ...
                    <div class="h-48 sm:h-56">
                      <canvas
                        ...
                        data-chart-datasets={Jason.encode!([%{data: Enum.map(@analytics.top_topics, & &1.count), backgroundColor: ["#6366f1","#8b5cf6","#a78bfa","#c4b5fd","#ddd6fe","#818cf8","#6d28d9","#5b21b6","#4c1d95","#312e81"]}])}
                      ></canvas>
                    </div>
                  </div>
                  ...
                  <%!-- Webhook stats --%>
                  <div class="border border-slate-200 rounded-lg p-3 sm:p-4">
                    <h3 class="text-sm font-semibold text-slate-700 mb-3"><%= gettext("Entregas por webhook (7d)") %></h3>
                      ...
                      <div class="space-y-2 max-h-56 overflow-y-auto">
                        <%= for ws <- @analytics.webhook_stats do %>
                          <div class="flex items-center gap-2 text-sm">
                            <span class="font-mono text-xs text-slate-600 truncate flex-1 min-w-0"><%= ws.webhook_url %></span>
            ...
            <section class="bg-white rounded-xl border border-teal-200 shadow-sm p-4 sm:p-6 overflow-hidden">
              ...
              <.form for={%{}} id="event-schema-form" phx-submit="create_event_schema" class="flex flex-col sm:flex-row gap-2 mb-4">
                <input type="text" name="topic" placeholder={gettext("Topic (ej: order.created)")} required class="flex-1 min-w-0 border border-slate-300 rounded-lg px-3 py-2 text-sm" />
                <textarea name="schema" rows="2" placeholder={gettext("JSON Schema (ej: {\"type\":\"object\",\"required\":[\"amount\"]})")} required class="flex-1 min-w-0 border border-slate-300 rounded-lg px-3 py-2 text-sm font-mono"></textarea>
                <button type="submit" phx-disable-with={gettext("Creando...")} class="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg text-sm font-medium shrink-0"><%= gettext("Crear") %></button>
              </.form>

              <%= if @event_schemas == [] do %>
                <p class="text-sm text-slate-500"><%= gettext("Sin schemas. Los eventos no serán validados.") %></p>
              <% else %>
                <div class="overflow-x-auto rounded-lg border border-slate-200">
                  <table class="min-w-full divide-y divide-slate-200">
                    <thead>
                      <tr class="bg-slate-50/80">
                        <th class="px-3 sm:px-4 py-2 text-left text-xs font-semibold text-slate-600 uppercase"><%= gettext("Topic") %></th>
                        <th class="px-3 sm:px-4 py-2 text-left text-xs font-semibold text-slate-600 uppercase hidden sm:table-cell"><%= gettext("Versión") %></th>
                        <th class="px-3 sm:px-4 py-2 text-left text-xs font-semibold text-slate-600 uppercase"><%= gettext("Estado") %></th>
                        <th class="px-3 sm:px-4 py-2 text-right text-xs font-semibold text-slate-600 uppercase"><%= gettext("Acciones") %></th>
                      </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                      <%= for s <- @event_schemas do %>
                        <tr class="hover:bg-slate-50/50 transition">
                          <td class="px-3 sm:px-4 py-2 text-sm text-slate-700 font-mono"><%= s.topic %></td>
                          <td class="px-3 sm:px-4 py-2 text-sm text-slate-600 hidden sm:table-cell">v<%= s.version %></td>
                          <td class="px-3 sm:px-4 py-2"><span class="px-2 py-0.5 rounded text-xs font-medium bg-teal-100 text-teal-800"><%= s.status %></span></td>
                          <td class="px-3 sm:px-4 py-2 text-right">
                            <button phx-click="delete_event_schema" phx-value-id={s.id} data-confirm={gettext("¿Eliminar este schema?")} class="text-red-600 hover:text-red-700 text-xs font-medium"><%= gettext("Eliminar") %></button>
                          </td>
                        </tr>
                      <% end %>
                    </tbody>
            ...
            <section class="bg-white rounded-xl border border-cyan-200 shadow-sm p-4 sm:p-6 overflow-hidden">
              ...
              <.form for={%{}} id="invite-member-form" phx-submit="invite_member" class="flex flex-col sm:flex-row gap-2 mb-4">
                <input type="email" name="email" placeholder={gettext("Email del usuario")} required class="flex-1 min-w-0 border border-slate-300 rounded-lg px-3 py-2 text-sm" />
                ...
                <button type="submit" phx-disable-with={gettext("Invitando...")} class="px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded-lg text-sm font-medium shrink-0"><%= gettext("Invitar") %></button>
              </.form>

              <%= if @team_members == [] do %>
                <p class="text-sm text-slate-500"><%= gettext("Solo tú tienes acceso. Invita colaboradores.") %></p>
              <% else %>
                <div class="overflow-x-auto rounded-lg border border-slate-200">
                  <table class="min-w-full divide-y divide-slate-200">
                    <thead>
                      <tr class="bg-slate-50/80">
                        <th class="px-3 sm:px-4 py-2 text-left text-xs font-semibold text-slate-600 uppercase"><%= gettext("Usuario") %></th>
                        <th class="px-3 sm:px-4 py-2 text-left text-xs font-semibold text-slate-600 uppercase"><%= gettext("Rol") %></th>
                        <th class="px-3 sm:px-4 py-2 text-left text-xs font-semibold text-slate-600 uppercase"><%= gettext("Estado") %></th>
                        <th class="px-3 sm:px-4 py-2 text-right text-xs font-semibold text-slate-600 uppercase"><%= gettext("Acciones") %></th>
                      </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                      <%= for m <- @team_members do %>
                        <tr class="hover:bg-slate-50/50 transition">
                          <td class="px-3 sm:px-4 py-2 text-sm text-slate-700 font-mono truncate max-w-[10rem] sm:max-w-none"><%= String.slice(m.user_id, 0, 8) %>...</td>
                          <td class="px-3 sm:px-4 py-2">
                            <span class={"px-2 py-0.5 rounded text-xs font-medium #{member_role_class(m.role)}"}><%= m.role %></span>
                          </td>
                          <td class="px-3 sm:px-4 py-2">
                            <span class={"px-2 py-0.5 rounded text-xs font-medium #{member_status_class(m.status)}"}><%= m.status %></span>
                          </td>
                          <td class="px-3 sm:px-4 py-2 text-right">
                            <%= if m.role != "owner" do %>
                              <div class="flex flex-col sm:flex-row sm:justify-end gap-1">
                                <%= if m.role == "viewer" do %>
                                  <button phx-click="update_member_role" phx-value-id={m.id} phx-value-role="editor" class="text-indigo-600 hover:text-indigo-700 text-xs font-medium"><%= gettext("Promover") %></button>
                                <% end %>
                                <button phx-click="remove_member" phx-value-id={m.id} data-confirm={gettext("¿Remover este miembro?")} class="text-red-600 hover:text-red-700 text-xs font-medium"><%= gettext("Remover") %></button>
                              </div>
                            <% end %>
                          </td>
                        </tr>
            ...
            <section class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 sm:p-6 overflow-hidden">
              <div class="flex items-center gap-2 mb-4">
                ...
                <h2 class="text-lg font-semibold text-slate-900"><%= gettext("Registro de actividad") %></h2>
              </div>
              <%= if @audit_logs == [] do %>
                      ...
                      <div class="border-t border-slate-200 pt-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                          ...
                          <div>
                            ...
                            <div class="flex gap-2">
                              <input type="text" name="schedule_cron" id="cron-expr-input" value={p["schedule_cron"]} placeholder="0 0 * * *" class="flex-1 border border-slate-300 rounded-xl px-4 py-2.5 text-slate-900 font-mono text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                              <button type="button" phx-click="preview_cron" phx-value-expression={p["schedule_cron"]} class="px-3 py-2 text-xs font-medium text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-xl transition shrink-0">
                                <%= gettext("Preview") %>
                              </button>
                            </div>
                            <%= if @cron_preview != [] do %>
                              <div class="mt-2 p-3 bg-slate-50 rounded-lg border border-slate-200">
                                <p class="text-xs font-medium text-slate-500 mb-1"><%= gettext("Próximas ejecuciones") %>:</p>
