defmodule StreamflixWebWeb.Api.V1.PlatformEventsController do
  ...
  operation(:index,
    ...
    parameters: [
      include: [
        in: :query,
        type: :string,
        description: "Set to 'inactive' to include inactive events"
      ],
    ...
    responses: [
      ok:
        {"Events list", "application/json",
         %OpenApiSpex.Schema{
           type: :object,
           properties: %{events: %OpenApiSpex.Schema{type: :array, items: Schemas.Event}}
         }}
    ]
  )
    ...
    case Platform.get_event(id) do
      ...
      event ->
        if event.project_id != project.id do
          ...
          json(
            conn,
            event_json(event) |> Map.put(:deliveries, Enum.map(deliveries, &delivery_mini/1))
          )
        end
    end
  end
  ...
  operation(:delete,
    ...
    responses: [
      ok:
        {"Event deactivated", "application/json",
         %OpenApiSpex.Schema{
           type: :object,
  ...
  operation(:simulate,
    ...
    responses: [
      ok:
        {"Simulation results", "application/json",
         %OpenApiSpex.Schema{
           type: :object,
           properties: %{
             simulation: %OpenApiSpex.Schema{type: :boolean},
             matching_webhooks: %OpenApiSpex.Schema{type: :integer},
             results: %OpenApiSpex.Schema{type: :array, items: %OpenApiSpex.Schema{type: :object}}
           }
         }},
      unprocessable_entity: {"Invalid payload", "application/json", Schemas.ErrorResponse}
    ]
