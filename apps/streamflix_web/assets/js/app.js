...
Hooks.VideoPlayer = {
  mounted() {
    ...
    loading.className = "absolute inset-0 z-5 flex items-center justify-center bg-black/60 opacity-0 pointer-events-none transition-opacity duration-200"
    loading.innerHTML = '<div class="w-14 h-14 border-4 border-white/30 border-t-white rounded-full animate-spin"/>'
    ...
    playBtn.type = "button"
    playBtn.className = "p-2 rounded-full hover:bg-white/20 transition cursor-pointer"
    ...
    range.min = 0
    range.step = 0.1
    range.className = "flex-1 h-2 bg-gray-600 rounded appearance-none cursor-pointer min-w-0"
    const timeLabel = document.createElement("span")
    timeLabel.className = "text-sm tabular-nums min-w-[90px] shrink-0 text-white"

    // Mute button
    const muteBtn = document.createElement("button")
    muteBtn.type = "button"
    muteBtn.className = "p-2 rounded-full hover:bg-white/20 transition shrink-0 cursor-pointer"
    muteBtn.title = "Silenciar"
    muteBtn.innerHTML = '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"/></svg>'
    const mutedSvg = '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z"/></svg>'
    ...
    volSlider.step = 0.05
    volSlider.value = 1
    volSlider.className = "w-20 h-1.5 bg-gray-600 rounded appearance-none cursor-pointer accent-red-600"

    // Speed button
    const speedBtn = document.createElement("button")
    speedBtn.type = "button"
    speedBtn.className = "px-2 py-1 rounded text-sm hover:bg-white/20 transition shrink-0 cursor-pointer text-white"
    speedBtn.textContent = "1x"

    // Fullscreen button
    const fullscreenBtn = document.createElement("button")
    fullscreenBtn.type = "button"
    fullscreenBtn.className = "p-2 rounded-full hover:bg-white/20 transition shrink-0 cursor-pointer"
    fullscreenBtn.title = "Pantalla completa"
    fullscreenBtn.innerHTML = '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"/></svg>'
    ...
    controlsBar.className = "absolute bottom-0 left-0 right-0 z-20 flex items-center gap-2 px-4 py-2 bg-gradient-to-t from-black/95 to-black/70 text-white transition-opacity duration-300"
    ...
    overlay.className = "absolute top-0 left-0 right-0 z-20 flex flex-col transition-opacity duration-300"
    const overlayRow = document.createElement("div")
    overlayRow.className = "flex items-center justify-between p-4 bg-gradient-to-b from-black/70 to-transparent"
    const volverWrap = document.createElement("div")
    const volverBtn = document.createElement("button")
    volverBtn.type = "button"
    volverBtn.className = "text-white hover:text-gray-300 flex items-center bg-black/50 hover:bg-black/70 px-3 py-2 rounded transition cursor-pointer"
    volverBtn.innerHTML = '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg><span class="ml-2">Volver</span>'
    ...
    titleEl.className = "text-white text-lg font-medium bg-black/50 px-4 py-2 rounded"
    ...
    if (nextUrl) {
      ...
      nextEpisodeWrap.className = "absolute bottom-20 right-6 z-30 pointer-events-none opacity-0 transition-opacity duration-300"
      const nextEpisodeBtn = document.createElement("button")
      nextEpisodeBtn.type = "button"
      nextEpisodeBtn.className = "pointer-events-auto px-4 py-2 rounded bg-white/90 hover:bg-white text-black font-medium text-sm flex items-center gap-2 transition cursor-pointer"
    ...
    endedOverlay.className = "absolute inset-0 z-40 hidden flex flex-col items-center justify-center bg-black/90"
    ...
    videoContainer.addEventListener("click", (e) => {
      if (e.target === videoContainer || e.target === video || e.target === loading) {
        showUI()
    ...
    video.addEventListener("waiting", () => { loading.classList.remove("opacity-0") })
    video.addEventListener("playing", () => { loading.classList.add("opacity-0") })
    video.addEventListener("canplay", () => { loading.classList.add("opacity-0") })

    video.addEventListener("ended", () => {
      if (Number.isFinite(video.duration) && video.duration > 0) {
        this.pushEvent("progress", { currentTime: video.duration, duration: video.duration })
      }
    ...
    muteBtn.addEventListener("click", (e) => {
      ...
      muteBtn.innerHTML = video.muted ? mutedSvg : '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"/></svg>'
    ...
    volSlider.addEventListener("input", (e) => {
      ...
      if (Number.isFinite(v)) {
        video.volume = v
        video.muted = v === 0
        muteBtn.innerHTML = v === 0 ? mutedSvg : '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"/></svg>'
      }
    })
    ...
    // Keyboard shortcuts
    const onKey = (e) => {
      if (!this.el.contains(document.activeElement) && document.activeElement?.tagName !== "BODY") return
