...
| **Rate Limiting Outbound** | Throttle outbound deliveries per webhook to protect receiver servers. Configurable `max_per_second` and `max_per_minute` per webhook. Deliveries are queued and retried automatically when limits are reached. Dashboard shows real-time throttle status. |
| **Prometheus Metrics** | `/metrics` endpoint (port 4021) compatible with Prometheus. Counters for events/deliveries/circuit breakers, gauges for active webhooks and pending queue, histograms for delivery latency. Labels by project, webhook, and topic. Ready for Grafana dashboards. |
| **Webhook Testing** | Send test events to webhooks from the dashboard or API (`POST /webhooks/:id/test`). No real event created — verifies endpoint connectivity, shows status code and latency in real time. |
| **Data Retention** | Configurable per-project retention policy (events, deliveries, audit logs) with automatic weekly purge. Manual selective purge with preview, filterable by type, date, topic, and status. |
