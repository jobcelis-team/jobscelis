defmodule StreamflixWebWeb.Api.V1.PlatformProjectsController do
  ...
  operation :create,
    ...
    responses: [created: {"Project created", "application/json", StreamflixWebWeb.Schemas.ProjectResponse}]
  ...
  operation :show,
    ...
    responses: [ok: {"Project details", "application/json", StreamflixWebWeb.Schemas.ProjectResponse}]
  ...
  def show(conn, %{"id" => id}) do
    ...
    case Platform.get_project(id) do
      ...
      project ->
        if project.user_id == user.id or StreamflixCore.Teams.user_can_access?(project.id, user.id) do
  ...
  operation :update,
    ...
    parameters: [id: [in: :path, type: :string, required: true]],
    request_body: {"Project params", "application/json", StreamflixWebWeb.Schemas.ProjectCreate},
    responses: [ok: {"Project updated", "application/json", StreamflixWebWeb.Schemas.ProjectResponse}]
      ...
      project ->
        if project.user_id == user.id or StreamflixCore.Teams.user_can_write?(project.id, user.id) do
          ...
          case Platform.update_project(project, attrs) do
            {:ok, updated} -> json(conn, %{data: project_json(updated)})
            {:error, changeset} ->
  ...
  operation :delete,
    ...
    parameters: [id: [in: :path, type: :string, required: true]],
    responses: [ok: {"Project deleted", "application/json", StreamflixWebWeb.Schemas.ProjectResponse}]
      ...
      project ->
        if project.user_id != user.id do
          ...
          case Platform.delete_project(project) do
            ...
            {:error, _} -> conn |> put_status(:unprocessable_entity) |> json(%{error: "Could not delete project"})
          end
        end
    end
  end
  ...
  def set_default(conn, %{"id" => id}) do
    ...
    case Platform.set_default_project(user.id, id) do
      ...
      {:error, _} -> conn |> put_status(:unprocessable_entity) |> json(%{error: "Could not set default"})
    end
  end
